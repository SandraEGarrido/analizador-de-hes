...
import { generateOrderExcel, generateAllExcelZip } from './services/excelService';
...
function convertirFechaExcel(fecha: any) {
	if (!fecha) return "";

	// Si ya viene como texto visible del Excel, no tocar
	if (typeof fecha === "string") {
		return fecha;
	}

	// Si viene como número serial de Excel, convertir sin corrimiento por zona horaria
	if (typeof fecha === "number") {
		const serial = Math.floor(fecha);
		const utcDays = serial - 25569;
		const utcValue = utcDays * 86400;
		const fechaJS = new Date(utcValue * 1000);
		...
		// Formato igual al archivo original: MM/DD/YYYY
		return `${dia}/${mes}/${anio}`;
	}
...
export default function App() {
	...
	const orderGroups = useMemo(() => {
		if (hesData.length === 0 || !isProcessing) return [];
		...
		return Object.entries(groups).map(([orderId, records]) => {
			...
			const totalAmount = currentRecords.reduce((sum, r) => sum + (Number(r['Importar HES']) || 0), 0);
			...
			const pedidoDuplicates: string[] = [];
			if (billedOrders.has(orderId)) {
				pedidoDuplicates.push('Historial');
			}
			...
			return {
				...
				totalAmount,
				hesCount: currentRecords.length,
				...
				hasExcessiveHES: currentRecords.length > 400,
				duplicates,
				pedidoDuplicates
			} as OrderGroup;
		});
	}, [hesData, billedOrders, threshold, invoiceNumbers, isProcessing]);
			...
			try {
				...
				const cleanedData = rawData
					...
					.map(row => {
						...
						const dateVal = convertirFechaExcel(row['Fecha HES']);
	...
	const resetAll = () => {
		...
		if (hesInputRef.current) {
			hesInputRef.current.value = '';
		}

		if (billedInputRef.current) {
			billedInputRef.current.value = '';
		}
	};

	const validateData = () => {
		// Requirement 6: Validation step to compare processed data against original file
		// Filter original data to remove empty rows or rows that are not HES records (headers/totals)
		const validOriginalRows = originalHesData.filter(row => {
			if (!row['N° HES']) return false;
			const rowValues = Object.values(row).map(v => String(v).toLowerCase());
			if (rowValues.some(v => v.includes('importe total') || v.includes('importación total'))) return false;
			return true;
		...
		if (validOriginalRows.length !== hesData.length) {
			setError(`Error de validación: La cantidad de registros procesados (${hesData.length}) no coincide con los registros válidos del archivo original (${validOriginalRows.length}).`);
			return false;
		}

		// Check order consistency
		for (let i = 0; i < hesData.length; i++) {
			const processed = hesData[i];
			const original = validOriginalRows[i];

			if (String(processed['N° HES']) !== String(original['N° HES']) ||
				String(processed['N° Pedido']) !== String(original['N° Pedido'])) {
				setError(`Error de validación: El orden de los datos ha sido alterado en el registro ${i + 1}.`);
				return false;
			}
		}
	...
	const hasBlockingIssues = (group: OrderGroup) => {
		return group.isDuplicate || group.duplicates.length > 0 || group.hasExcessiveHES;
	};
	const handleDownload = (format: 'pdf' | 'excel') => {
		if (!validateData()) return;

		const { order, all } = showDownloadModal;

		if (all) {
			const validOrders = orderGroups.filter(group => !hasBlockingIssues(group));

			if (validOrders.length === 0) {
				setError('No hay reportes válidos para descargar. Todos los pedidos tienen alertas o errores.');
				setShowDownloadModal({ open: false });
				return;
			}

			if (format === 'pdf') {
				generateAllPDFsZip(validOrders);
			} else {
				generateAllExcelZip(validOrders);
			}
		} else if (order) {
			if (format === 'pdf') {
				generateOrderPDF(order);
			} else {
				generateOrderExcel(order);
			}
		}
		...
		<div className="min-h-screen bg-[#F8FAFC] text-[#1E293B] font-sans selection:bg-indigo-100">
			...
			<header className="bg-white border-b border-slate-200 sticky top-0 z-50">
				<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
					...
					<div className="flex items-center gap-4">
						{isProcessing && orderGroups.length > 0 && (
							<>
								<button
									onClick={() => setShowDownloadModal({ open: true, all: true })}
						...
						<button
							...
							className="group flex flex-col items-center justify-center gap-1 min-w-[140px] px-4 py-3 rounded-2xl bg-white border border-slate-200 text-slate-700 shadow-sm hover:bg-indigo-600 hover:text-white hover:border-indigo-600 hover:shadow-lg hover:shadow-indigo-200 transition-all active:scale-95"
						>
					...
					<aside className="lg:col-span-4 space-y-6">
									...
									<div className={cn(
										...
										{billedOrders.size > 0 ? (
											...
											<div className="flex flex-col items-center gap-2">
												...
												<span className="text-sm font-medium text-slate-600">Subir Historial</span>
						...
						<section className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
							...
							<ul className="text-xs text-slate-500 space-y-2 list-disc pl-4">
								...
								<li>Descarga el informe individual (Excel o PDF) para cada pedido procesado.</li>
							</ul>
						</section>
					</aside>
							...
							{hesData.length === 0 ? (
								...
								>
									...
									<p className="text-slate-500 max-w-md mx-auto">
										Sube tu archivo Excel con las HES para analizar los pedidos, verificar límites y generar los informes en Excel o PDF.
									</p>
								</motion.div>
							) : (
								<motion.div
													...
													<div className="space-y-1">
														<div className="flex items-center gap-2">
															...
															<span className="bg-slate-100 text-slate-600 text-[10px] px-2 py-0.5 rounded-full font-bold uppercase">
																Cantidad de HES: {group.hesCount}
															</span>
															{group.isDuplicate && (
													...
													<div className="flex items-center gap-3">
														<div className="relative">
															<input
																...
																className="w-64 px-3 py-2 text-sm rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
															/>
														</div>
														<button
															onClick={() => setShowDownloadModal({ open: true, order: group })}
													...
													<div className="mt-4 pt-4 border-t border-slate-100 space-y-2">
														{group.isDuplicate && (
															<p className="text-xs text-red-600 flex items-center gap-1.5 font-medium">
																...
																Este pedido ya fue facturado anteriormente (Historial). No se puede generar el informe.
															</p>
														)}
														{group.pedidoDuplicates.filter(d => d !== 'Historial').map((dup, idx) => (
															<p key={idx} className="text-xs text-red-600 flex items-center gap-1.5 font-medium">
																<AlertTriangle size={14} />
																Pedido duplicado detectado en: {dup}
															</p>
														))}
			...
			{showDownloadModal.open && (
				<div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
					...
					>
						<h3 className="text-xl font-bold text-slate-900 mb-2">¿En qué formato desea descargar los informes?</h3>
						<p className="text-slate-500 text-sm mb-6">Seleccione el formato de archivo para su descarga.</p>

						<div className="grid grid-cols-2 gap-4">
							<button
								onClick={() => handleDownload('pdf')}
								className="flex flex-col items-center gap-3 p-6 rounded-2xl border-2 border-slate-100 hover:border-red-200 hover:bg-red-50 transition-all group"
							>
							...
							<button
								...
								className="flex flex-col items-center gap-3 p-6 rounded-2xl border-2 border-slate-100 hover:border-emerald-200 hover:bg-emerald-50 transition-all group"
							>
						...
						<button
							onClick={() => setShowDownloadModal({ open: false })}
							className="w-full mt-6 py-3 text-slate-400 font-medium hover:text-slate-600 transition-colors"
						>
							Cancelar
						</button>
					</motion.div>
